...
describe('RingManager', () => {
    ...
    beforeEach(() => {
        scene = new MockScene();
        drone = { position: new THREE.Vector3() };
        colliderSystem = new MockColliderSystem();
        // RingManager calls spawnRing() in constructor, which uses Math.random
        // We might want to control random or just verify side effects
    ...
    it('should collect ring when drone flies through center', () => {
        ...
        // Position ring at (10, 10, 10) facing Z (default rotation logic is random, so we must force it)

        // Force ring position/rotation for deterministic test
        ring.mesh.position.set(10, 10, 10);
        ring.mesh.rotation.set(0, 0, 0); // Facing Z? Torus is XY plane.
        // If rotation is 0,0,0, Torus is in XY plane. Normal is Z.
        // To fly "through", we need to move along Z.
    ...
    it('should NOT collect ring if hitting the rim (outside hole)', () => {
        ...
        // Logic check: distToCenter < 1.0 is the condition in code
        // 1.4 is > 1.0, so should not collect.
