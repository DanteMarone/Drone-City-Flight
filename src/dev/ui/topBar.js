...
export class TopBar {
    ...
    _createMenu(parent, label, items) {
        ...
        items.forEach(item => {
            if (item.separator) {
                ...
                // Simple keyboard navigation (Arrow keys would be better but this is a start)
                button.onkeydown = (e) => {
                    if (e.key === 'Escape') {
                        this._closeAllMenus();
                        btn.focus();
    ...
    _triggerLoad() {
        ...
        fileInput.className = 'visually-hidden'; // Use the CSS class instead of inline style
