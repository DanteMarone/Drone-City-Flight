...
export class PropertiesInspector {
    ...
    render(container) {
        ...
        if (selection.length === 1) {
            container.appendChild(createPropGroup('Transform', [
                ...
                createScaleInput(
                    ...
                    (axis, val) => this._handleScaleChange(obj, axis, val)
                )
            ]));
        } else {
            ...
            if (proxy) {
                container.appendChild(createPropGroup('Transform (Group)', [
                    ...
                    createScaleInput(
                        ...
                        this.lockScale,
                        (locked) => this.lockScale = locked,
                        (axis, val) => this._handleScaleChangeProxy(proxy, axis, val)
                    )
                ]));
            } else {
        ...
        if (this.alignTool && this.alignTool.createUI) {
            ...
            if (alignUI) {
                container.appendChild(createPropGroup('Alignment', [alignUI]));
            }
        }
            ...
            if (obj.userData.params) {
                ...
                Object.keys(obj.userData.params).forEach(key => {
                    ...
                    const isColor = key.toLowerCase().includes('color') ||
                                    ...
                                    (typeof val === 'number' && key.toLowerCase().includes('color'));
    ...
    _handleScaleChangeProxy(proxy, axis, n) {
        const vec = proxy.scale;
        if (this.lockScale) {
            const newVec = new THREE.Vector3(n, n, n);
    ...
    _applyTransform(obj, prop, val) {
        ...
        if (this.devMode.gizmo) this.devMode.gizmo.attach(obj);
    ...
    _renderWaypoints(container, obj) {
