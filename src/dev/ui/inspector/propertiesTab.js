...
export class InspectorPropertiesTab {
    ...
    render(content) {
        ...
        if (!selection || selection.length === 0) {
            content.innerHTML = '<div style="color:#666; font-style:italic;">No object selected</div>';
            return;
        }
        ...
        if (selection.length === 1) {
            const transformGroup = createPropGroup('Transform', [
                createVectorInput('Position', obj.position, (v) => this.applyTransform(obj, 'position', v)),
                createVectorInput('Rotation', obj.rotation, (v) => this.applyTransform(obj, 'rotation', v), true),
                createScaleInput(
                    obj.scale,
                    () => this.lockScale,
                    (val) => this.lockScale = val,
                    (v) => this.applyTransform(obj, 'scale', v)
                )
            ]);
            content.appendChild(transformGroup);
        } else {
            // Group Selection - Use Proxy
            const proxy = this.devMode.gizmo ? this.devMode.gizmo.proxy : null;
            if (proxy) {
                const transformGroup = createPropGroup('Transform (Group)', [
                    createVectorInput('Position', proxy.position, (v) => this.applyProxyTransform('position', v)),
                    createVectorInput('Rotation', proxy.rotation, (v) => this.applyProxyTransform('rotation', v), true),
                    createScaleInput(
                        proxy.scale,
                        () => this.lockScale,
                        (val) => this.lockScale = val,
                        (v) => this.applyProxyTransform('scale', v)
                    )
                ]);
                content.appendChild(transformGroup);
            } else {
                content.innerHTML += '<div style="color:#888;">Use Gizmo to transform selection.</div>';
            }
        }
        ...
        if (this.alignTool && this.alignTool.createUI) {
            ...
            if (alignUI) {
                content.appendChild(createPropGroup('Alignment', [alignUI]));
            }
        }
    ...
    sync() {
        ...
        if (selection.length === 1) {
            const obj = selection[0];
            syncVectorInput('Position', obj.position);
            syncVectorInput('Rotation', obj.rotation, true);
            syncVectorInput('Scale', obj.scale);
        } else {
             // Multi-select: Sync from Proxy
             const proxy = this.devMode.gizmo ? this.devMode.gizmo.proxy : null;
             if (proxy) {
                 syncVectorInput('Position', proxy.position);
                 syncVectorInput('Rotation', proxy.rotation, true);
                 syncVectorInput('Scale', proxy.scale);
             }
        }
    }

    applyTransform(obj, prop, val) {
        // Snapshot before
        const before = [{
            object: obj,
